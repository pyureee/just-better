...
const data = JSON.parse(fs.readFileSync(root + '/skills/female/elin/Priest.json'));
let now = 10000;
...
function harness(speed=2,autoBlock=false) {
  now = 10000;
  let nextTimer = 0;
  const timers = new Map(), hooks = [], sent = [], commands = {}, errors = [], messages=[];
  const abnormalities=new Map();
  const mod = {
  ...
  const mods = {
    ...
    player: {job: 6, mp:10000, inCombat: true, alive: true, gameId: 1n, stamina: 10000, inven: {weapon}, isMe: id => id === 1n},
    ping: {ping: 157, jitter: 10}, packet: {get_all: name => [name, 1]},
    effects: {
      getAbnormality: id => abnormalities.get(id) || null,
    ...
    command: {add: (name, fn) => commands[name] = fn, remove: name => delete commands[name], message: x => messages.push(x)},
    library: {jsonStringify: obj => JSON.stringify(obj, (key, value) => typeof value === 'bigint' ? value.toString() : value)},
    utils: {
      isEnabled: id => !disabled && (id === undefined || mods.skills.isSupported(id)), canCastSkill: () => true, getPacketBuffer: () => 350,
  ...
  mod.hook('S_ACTION_STAGE',1,{order:96,filter:{fake:true}},event=>{for(const a of data[event.skill.id]?.abnormalityApply||[])abnormalities.set(a.id,{id:a.id,time:now});});
  const Precast = require(process.env.TEST_INSTALLED ? root+'/libs/user_plugins/priest_entry_skill' : root+'/libs/user_plugins/priest_entry_skill');
  ...
  return {mod,mods,plugin,emulation,blockPlugin,input,press,cd,advance,dispatch,sent,timers,errors,commands,messages,abnormalities,
